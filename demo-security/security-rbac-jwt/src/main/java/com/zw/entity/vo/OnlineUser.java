...
public class OnlineUser {
    ...
    public static OnlineUser create(SecUser user) {
        OnlineUser onlineUser = new OnlineUser();
        BeanUtil.copyProperties(user, onlineUser);
        // 脱敏
        onlineUser.setPhone(StrUtil.hide(user.getPhone(), 3, 7));
        onlineUser.setEmail(StrUtil.hide(user.getEmail(), 1, StrUtil.indexOfIgnoreCase(user.getEmail(), ParamKey.SYMBOL_EMAIL)));
        return onlineUser;
    }
}
